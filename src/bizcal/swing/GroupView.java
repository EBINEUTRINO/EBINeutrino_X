...
//        scrollPane.setCorner(JScrollPane.UPPER_RIGHT_CORNER, createCorner(false, true));
    ...
    public void refresh0() throws Exception {
        ...
        while (i.hasNext()) {
            ...
            while (j.hasNext()) {
                ...
                frameAreas.add(area);
                calPanel.add(area, new Integer(event.getLevel() < 0 ? 0 : event.getLevel()));
            }
        }
        ...
        while (i.hasNext()) {
            bizcal.common.BCalendar calendar = (bizcal.common.BCalendar) i.next();
    ...
    protected Object getCalendarId(int x, int y)
        ...
        if (pos >= getSelectedCalendars().size())
            return null;
        bizcal.common.BCalendar cal =
                (bizcal.common.BCalendar) getSelectedCalendars().get(pos);
        return cal.getId();
    }
